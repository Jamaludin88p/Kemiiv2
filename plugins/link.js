let handler = async (m, { conn, args }) => {
	let id = m.chat
      ...
      //  if (!/^[0-9]{5,16}-[0-9]+@g\.us$/.test(id)) throw 'Hanya bisa dibuka di grup'
        ...
        if (!groupMetadata) return conn.reply(m.chat, 'groupMetadata is undefined', m)
        if (!'participants' in groupMetadata) return conn.reply(m.chat, 'participants is not defined', m)
 
        conn.reply('https://chat.whatsapp.com/' + await conn.groupInviteCode(id), m)
/*
    if (!m.isGroup) {
